...
{
    m_cameraParameters = CameraParameterListModel::parseParameterFile(fileName);
}
...
{
    return m_cameraParameters.count();
}
...
QVariant CameraParameterListModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.row() >= m_cameraParameters.size())
        return QVariant();
    ...
    if (role == Qt::UserRole)
    {
        ...
        cameraParameterSet.setValue(m_cameraParameters.at(index.row()));
        return cameraParameterSet;
    }

    // pass only name for use by the gui
    if (role == Qt::DisplayRole)
        return m_cameraParameters.at(index.row()).getCamName();
    else
        return QVariant();
}

bool CameraParameterListModel::replaceListFromParameterFile(const QString &fileName)
{
    QList<CameraParameterSet> cameraParameters;
    QFile paramFile(fileName);
    paramFile.open(QIODevice::ReadOnly);
...
//        exMat_dst_tmp(Range(0, 3), Range(3, 4)) = -R*t; // overwrite translation part
    ...
    // update model: remove old data
    beginRemoveRows(QModelIndex(),0,m_cameraParameters.count());
    m_cameraParameters.clear();
    endRemoveRows();
    // update model: set new data
    beginInsertRows(QModelIndex(),0,cameraParameters.count());
    m_cameraParameters.append(cameraParameters);
    endInsertRows();
